...
import Foundation
  ...
  @objcMembers public final class UI: BlocklyEvent {
      ...
      newValue: String? = nil)
    {
      ...
      super.init(type: UI.EVENT_TYPE, workspaceID: workspace.uuid, groupID: nil,
                 blockID: block?.uuid)
    ...
    public init(json: [String: Any]) throws {
      ...
      try super.init(type: UI.EVENT_TYPE, json: json)
      ...
      if [UI.elementClick,
          UI.elementCommentOpen,
          UI.elementMutatorOpen,
          UI.elementSelected,
          UI.elementTrashOpen,
          UI.elementWarningOpen].contains(element) &&
         (blockID?.isEmpty ?? true) {
        throw BlocklyError(.jsonParsing,
          "UI element \"\(element)\" requires that \"\(BlocklyEvent.JSON_BLOCK_ID)\" be assigned.")
      }

      if [UI.elementCommentOpen,
          UI.elementMutatorOpen,
          UI.elementSelected,
          UI.elementTrashOpen,
          UI.elementWarningOpen].contains(element) &&
     ...
     */
    public static func blockClickedEvent(workspace: Workspace, block: Block) -> UI {
      return UI(element: elementClick, workspace: workspace, block: block)
    }
    ...
    public static func blockSelectedEvent(
      ...
      -> UI
    {
      return UI(element: elementSelected, workspace: workspace, block: block,
    ...
    public static func blockWarningEvent(
      workspace: Workspace, block: Block, openedBefore: Bool, openedAfter: Bool) -> UI
    {
      return UI(element: elementWarningOpen, workspace: workspace, block: block,
    ...
    public static func commentEvent(
      workspace: Workspace, block: Block, openedBefore: Bool, openedAfter: Bool) -> UI
    {
      return UI(element: elementCommentOpen, workspace: workspace, block: block,
                             oldValue: openedBefore ? "true" : "false",
                             newValue: openedAfter ? "true" : "false")
    }
    ...
    public static func mutatorPopoverEvent(
      workspace: Workspace, block: Block, openedBefore: Bool, openedAfter: Bool) -> UI
    {
      return UI(element: elementMutatorOpen, workspace: workspace, block: block,
                             oldValue: openedBefore ? "true" : "false",
                             newValue: openedAfter ? "true" : "false")
    }
    ...
    public static func toolboxCategoryEvent(
      workspace: Workspace, oldValue: String?, newValue: String?) -> UI
    {
      return UI(
