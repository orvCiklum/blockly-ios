...
extension Field {
  ...
  */
  internal static func makeField(json: [String: Any]) throws -> Field? {
    ...
    if let creationHandler = JSONRegistry.shared[type] {
...
@objcMembers public final class JSONRegistry: NSObject {
  ...
  private override init() {
    ...
    // Angle
    registerType(Field.FIELD_TYPE_ANGLE) {
      ...
      let name = (json[Field.PARAMETER_NAME] as? String ?? "NAME")
      let angle = JSONRegistry.parseDouble(json[Field.PARAMETER_ANGLE]) ?? 90
      return FieldAngle(name: name, angle: angle)
    }
    
    // Checkbox
    registerType(Field.FIELD_TYPE_CHECKBOX) {
      (json: [String: Any]) throws -> Field in
      return FieldCheckbox(
        name: (json[Field.PARAMETER_NAME] as? String ?? "NAME"),
        checked: (json[Field.PARAMETER_CHECKED] as? Bool ?? true))
    }
    
    // Color
    registerType(Field.FIELD_TYPE_COLOR) {
      (json: [String: Any]) throws -> Field in
      let colorString = json[Field.PARAMETER_COLOR] as? String ?? ""
    ...
    // Date
    registerType(Field.FIELD_TYPE_DATE) {
      (json: [String: Any]) throws -> Field in
      return FieldDate(
        name: (json[Field.PARAMETER_NAME] as? String ?? "NAME"),
        stringDate: (json[Field.PARAMETER_DATE] as? String ?? ""))
    }
    
    // Dropdown
    registerType(Field.FIELD_TYPE_DROPDOWN) {
      (json: [String: Any]) throws -> Field in
      // Options should be an array of string arrays.
      // eg. [["Name 1", "Value 1"], ["Name 2", "Value 2"]]
      let options = json[Field.PARAMETER_OPTIONS] as? Array<[String]> ?? []
    ...
    // Image
    registerType(Field.FIELD_TYPE_IMAGE) {
      (json: [String: Any]) throws -> Field in
      return FieldImage(
        name: (json[Field.PARAMETER_NAME] as? String ?? ""),
        imageLocation: (Block.decodedJSONValue(json[Field.PARAMETER_IMAGE_URL]) as? String ??
          "https://www.gstatic.com/codesite/ph/images/star_on.gif"),
        size: WorkspaceSize(
          width: CGFloat((json[Field.PARAMETER_WIDTH] as? Int) ?? 15),
          height: CGFloat((json[Field.PARAMETER_HEIGHT] as? Int) ?? 15)),
        altText: (Block.decodedJSONValue(json[Field.PARAMETER_ALT_TEXT]) as? String ?? "*"),
        flipRtl: (json[Field.PARAMETER_IMAGE_FLIP_RTL] as? Bool ?? false))
    }
    
    // Input
    registerType(Field.FIELD_TYPE_INPUT) {
      (json: [String: Any]) throws -> Field in
      return FieldInput(
        name: (json[Field.PARAMETER_NAME] as? String ?? "NAME"),
        text: (Block.decodedJSONValue(json[Field.PARAMETER_TEXT]) as? String ?? "default"))
    }
    
    // Label
    registerType(Field.FIELD_TYPE_LABEL) {
      (json: [String: Any]) throws -> Field in
      return FieldLabel(
        name: (json[Field.PARAMETER_NAME] as? String ?? ""),
        text: (Block.decodedJSONValue(json[Field.PARAMETER_TEXT]) as? String ?? ""))
    }
    
    // Number
    registerType(Field.FIELD_TYPE_NUMBER) {
      (json: [String: Any]) throws -> Field in
      
      let value = JSONRegistry.parseDouble(json[Field.PARAMETER_VALUE]) ?? 0.0
      let minimum = JSONRegistry.parseDouble(json[Field.PARAMETER_MINIMUM_VALUE])
      let maximum = JSONRegistry.parseDouble(json[Field.PARAMETER_MAXIMUM_VALUE])
      let precision = JSONRegistry.parseDouble(json[Field.PARAMETER_PRECISION])
      
      let fieldNumber = FieldNumber(name: (json[Field.PARAMETER_NAME] as? String ?? ""), value: value)
      try fieldNumber.setConstraints(minimum: minimum, maximum: maximum, precision: precision)
      return fieldNumber
    }
    
    // Variable
    registerType(Field.FIELD_TYPE_VARIABLE) {
      (json: [String: Any]) throws -> Field in
      return FieldVariable(
        name: (json[Field.PARAMETER_NAME] as? String ?? "NAME"),
        variable: (Block.decodedJSONValue(json[Field.PARAMETER_VARIABLE]) as? String ?? "item"))
    }
  }
